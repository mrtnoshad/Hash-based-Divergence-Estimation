...
# Hash-based Divergence Estimator
#
# Created by Morteza Noshad (noshad@umich.edu)
#
# Based on the paper: Hash-based Divergence Estimator
#
################
...
def gen_W(X):
	...
	#np.random.seed(seed=0)
	W = np.random.normal(mu_X, sigma_X, (dim_X, d_shrink))
...
def Hash(XW,YV,eps,b):
	...
	for i in range(N):
		...
		X_l, Y_l = H1(XW[i],b,eps), H1(YV[i],b,eps)
...
# Compute divergence
def Compute_DI(XW,YV,U,eps,b):
	N = XW.shape[0]

	(CX, CY) = Hash(XW,YV,eps,b)
	...
	for e in CX.keys():
		...
		if Mj>0: 
			I += Ni*max(min(math.log(1.0*Ni/Mj,2), U),-U)
		else:
			I += Ni*U
...
def HBDE(X,Y,U=10, gamma=1, epsilon=0, epsilon_vector = 'fixed', eps_range_factor=0.1, normalize_epsilon = False ,
				ensemble_estimation = 'median', L_ensemble=1 ,hashing='p-stable', stochastic = False):
...
## Hash type
	...
	if hashing == 'p-stable':	
		...
		W = gen_W(X)
		#d_shrink, d_Y_shrink=W.shape[1], V.shape[1]
		d_shrink = W.shape[1]
		# Find inner products
		XW, YV = np.dot(X,W), np.dot(Y,W)
...
## Initial epsilon and apply smoothness gamma
	...
	if epsilon ==0:
		...
		eps_X_temp = gen_eps(XW)

		# Normalizing factors for the bandwidths
		cx= 3*d_shrink
		eps_X0= eps_X_temp * cx*gamma
	else:
		eps_X_temp = np.ones(d_shrink,)*epsilon
		cx = 3*d_shrink
		eps_X0= eps_X_temp * cx*gamma
...
## Compute MI Vector
	...
	for j in range(L_ensemble):
		...
		if stochastic== True:
			np.random.seed()
			f=0.1
			b = f*np.random.rand(d_shrink,)*eps_X
		else:
			b = np.linspace(0,1,L_ensemble,endpoint=False)[j]*eps_X

		I_vec[j] = Compute_DI(XW,YV,U,eps_X,b)
...
## Normalize epsilon according to MI estimation (cross validation)
	if normalize_epsilon == True:
		...
		I = HBDE(X,Y,U, gamma, epsilon, epsilon_vector, eps_range_factor, normalize_epsilon, ensemble_estimation, L_ensemble,hashing, stochastic)
...
# Fully Dependent Datasets
	...
	Y = 0.001*np.ones((N,d))+np.random.randn(N,d)
